...
class Encoder(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, num_layers=2, batch_first=True):
        ...
        self.lstm = nn.LSTM(self.hidden_size, self.hidden_size, self.hidden_size).to(device='cuda')

    # perform a forward pass through the network
    def forward(self, input_size, hidden):
        output = self.embedding(input_size).view(1, 1, -1).to(device = 'cuda')
        output, (hidden,cellstate) = self.lstm(output, (hidden, hidden))

        return output, (hidden, cellstate)[0]

    # initialise the hidden layer of the network

    def initHidden(self):
        return torch.zeros(300, 1, self.hidden_size, device=device).to(device = 'cuda')
...
class Decoder(nn.Module):
    ...
    def forward(self, input_size, hidden, encoder_outputs):
        ...
        return output, (hidden,cellstate)[0], attn_weights
...
if __name__ == '__main__':
    ...
    sample(encoder, decoder, input_text_representation,
